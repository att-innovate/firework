...
First, let's see how the six `CodedOutputStream::Write*()` methods identified in the last section are translated into a hardware accelerator design. Then, we'll integrate the newly developed FPGA peripheral into the larger <a href="https://rocketboards.org/foswiki/Documentation/GSRDGhrd">Arria 10 GHRD</a> system, where it'll serve as a co-processor to the ARM Cortex-A9 CPU on board.

### 4. Design and Implement the hardware accelerator (FPGA peripheral)

I claimed that last section was *the most important step* of the entire process. Well, I think this section is *the most challenging and interesting step* as much of it relies on piecing together information from various sources, problem solving, and creativity. Before I could begin designing the hardware accelerator (i.e., the top-level I/O, FSMs, and datapath using the `CodedOutputStream::Write*()` methods as templates), I needed to first understand **how it would integrate into the larger Arria 10 SoC** such that I've established communication with the ARM Cortex-A9 CPU and could interact with the FPGA design via software running in a Linux environment.

If you've worked with <a href="https://en.wikipedia.org/wiki/Field-programmable_gate_array">FPGAs</a> in the past to build simple digital designs, perhaps in the lab section of an introductory course on logic design, it's important to understand that building a hardware accelerator requires a different train of thought. It's not a *standalone piece of hardware* that we're implementing in the FPGA (e.g., a counter whose input is connected to a push button and output is displayed on a <a href="https://en.wikipedia.org/wiki/Seven-segment_display">7-segment display</a>); we're designing an FPGA peripheral that's just *one small component of a much larger, complicated digital system*. Specifically, we're designing a hardware accelerator that performs *Protocol Buffer serialization* and communicates with an ARM Cortex-A9 CPU, where the ARM CPU in turn executes a Protocol Buffer application in a Linux environment. In this configuration, the hardware accelerator serves as a *co-processor* to the ARM CPU and is viewed as a *memory-mapped FPGA peripheral* from its perspective. What differentiates this FPGA design from other standalone projects is that, aside from the complexity, its top-level I/O ports are solely connected to the <a href="https://www.altera.com/products/design-software/fpga-design/quartus-prime/features/qts-platform-designer.html">system interconnect</a> which enables this peripheral's *discovery* and *access* by the ARM CPU! It may be troubling at frist to think you're going to build a very sophisticated digital circuit whose only means of interaction is via instructions executed on an ARM CPU (sorry, no <a href="https://en.wikipedia.org/wiki/DIP_switch">dip switches</a> or LEDs here!), but rest assured, I'll show you how to methodically approach the problem and make it seem less like a leap of faith, like it was for me.

How did I come to realize that this is how the hardware accelerator would eventually fit within the larger Arria 10 SoC? It required a lot of patience and piecing together information from various sources. The following list, although not comprehensive, identifies separate initiatives I undertook and later realized were key contributors to obtaining the understanding I needed to begin designing the hardware accelerator:
- going through an extensive amount of Intel's <a href="https://www.altera.com/support/training/curricula.html">online training</a> covering everything from the general FPGA design process to how their EDA tools are used at the various stages of design, simulation, and testing (functional verificaiton, static timing analysis)
- fundamentally understanding the <a href="https://rocketboards.org/foswiki/Documentation/GSRDGhrd">Arria 10 GHRD's</a> architecture, design files, and how its example FPGA peripherals are accessed from Linux (and also realizing that the GHRD would be the starting point for my own *Qsys system design*)
- learning about the *FPGA-to-HPS*, *lightweight HPS-to-FPGA*, and *HPS-to-FPGA* interfaces which connect the Arria 10 SoC's <a href="https://www.altera.com/products/soc/portfolio/arria-10-soc/arria10-soc-hps.html">HPS</a> to soft IP (i.e., FPGA peripherals) residing in the FPGA fabric along with the <a href="https://en.wikipedia.org/wiki/Address_space">address spaces</a> reserved for them. I also had to learn how to assign addresses to FPGA components of a <a href="https://www.altera.com/products/design-software/fpga-design/quartus-prime/features/qts-platform-designer.html">Qsys system</a> such that the HPS could interact with them via one of these interfaces
- going through the <a href="https://rocketboards.org/foswiki/Documentation/AlteraSoCWorkshopSeries">Altera SoC Workshop Series</a> to learn about software development for Altera SoCs and how Linux device drivers are written for memory-mapped FPGA peripherals
- and finally, reading the <a href="https://github.com/att-innovate/firework/blob/master/resources/AMBA%20AXI%20and%20ACE%20Protocol%20Specification.pdf">ARM AMBA AXI protocol specification</a> in order to understand the signals that would comprise the hardware accelerator's top-level I/O ports and establish communication with ARM CPU

The remainder of this section is broken down into three main subsections. First, I'll list the relevant Intel online training, examples, and other available resources that led to understanding how the hardware accelerator would fit into the larger Arria 10 SoC. I'll conclude this subsection by discussing one training in particular that put all the pieces together, allowing me to begin designing the hardware accelerator. Next, I'll discuss the RTL design of the hardware accelerator in quite some detail and describe how it implements an ARM AMBA AXI4 slave interface, establishing communication with an ARM CPU. Finally, I'll walk you through using Intel's EDA tools to implement the hardware accelerator RTL design and run gate-level simulations for functional verification.

#### Training that led to understanding how the hardware accelerator fits into the Arria 10 SoC
...
##### Intel FPGA and other relevant online training
