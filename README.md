...
There are powerful tools available that I used to pinpoint the source code responsible for *Protocol Buffer serialization* and understand its role to an extent at which I was able to begin designing the specialized hardware that would eventually replace it. One such tool is the marriage of <a href="http://www.vim.org/">vim</a> and <a href="http://ctags.sourceforge.net/">ctags</a>; I used this combination to review the example application and jump between source files containing the definitions of class methods as they were invoked. This helped me better understand the relationship between the *compiler-generated code* and `libprotobuf.so.10.0.0` *runtime library* that are components of any Protocol Buffer application. I used another tool, <a href="https://www.gnu.org/software/gdb/">gdb</a>, to step through the same application and inspect <a href="https://en.wikipedia.org/wiki/Stack_trace">stack traces</a> as it actively serialized a Protocol Buffer <a href="https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.message#Message">Message</a>. This helped me understand the sequence (and frequency) of method invocations involved in serialization. The last tool I'll mention is <a href="https://perf.wiki.kernel.org/index.php/Main_Page">perf</a> - a powerful <a href="https://en.wikipedia.org/wiki/Profiling_(computer_programming)">profiling</a> tool used in <a href="https://en.wikipedia.org/wiki/Performance_tuning">performance tuning</a>. `perf` makes use of <a href="https://en.wikipedia.org/wiki/Hardware_performance_counter">performance counters, kernel <a href="https://www.kernel.org/doc/Documentation/trace/tracepoints.txt">tracepoints</a>, even <a href="http://www.brendangregg.com/perf.html#USDT">user-level statically defined tracing (USDT)</a>, and more to periodically sample an application as it runs and provide detailed reports on where it spends its time (i.e., the various code paths taken and percentage of total execution time they account for). As demonstrated in the section [Profiling the HW-accelerated system](README.md#9-profiling-the-hw-accelerated-system), we use it to profile a Protocol Buffer applicaiton, acquire performance statistics, and analyze differences in its execution by the standard (CPU/software) and HW-accelerated systems.
    
In the remainder of this section, I'll provide an overview of the Protocol Buffer software, demonstrate how I used the `vim`+`ctags` and `gdb` tools to identify source files, classes, and methods relevant to *Protocol Buffer serialization*, and discuss how time spent analyzing the `WireFormatLite` and <a href="https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.io.coded_stream#CodedOutputStream">CodedOutputStream</a> classes and various <a href="https://developers.google.com/protocol-buffers/docs/proto3#scalar">field types</a> led to a key realization and simplifcation of the hardware accelerator design. I'll conclude the section with a brief discussion about importance of using `perf` at this stage too.

#### Overview of Protocol Buffers
From the <a href="https://developers.google.com/protocol-buffers/docs/overview">Developer Guide</a>, "Protocol buffers are a flexible, efficient, automated mechanism for serializing structured data". The idea is that you define your data structures once in a `.proto` file, use the Protocol Buffer compiler (`protoc`) to generate specialized code in the language of your choice (e.g., C++), and use the compiler-generated code for easily accessing and serializing/parsing these data structures in your application. The compiler-generated code works closely with the Protocol Buffer *runtime library* to serialize/parse data structures to/from raw bytes. The data structures you define are called *messages*, and they consist of a series of name-value pairs called *fields*, similar to <a href="http://www.json.org/">JSON objects</a>. Protocol Buffers are *extensible* such that you can add new fields to messages without disrupting existing applications that use older formats; this is achieved by marking new fields as `optional` instead of `required`. 

For a more complete understanding of how Protocol Buffers are used, I recommend going through the following material before continuing:
...
- <a href="https://developers.google.com/protocol-buffers/docs/encoding">Encoding</a>: varint encoding; describes how messages encoded

Let's look more closely at the various message fields and their corresponding wire types. The following table from the <a href="https://developers.google.com/protocol-buffers/docs/encoding">encoding</a> page shows the mapping between *field types* and the six available *wire types*:
