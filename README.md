...
If you've worked with <a href="https://en.wikipedia.org/wiki/Field-programmable_gate_array">FPGAs</a> in the past, perhaps in the lab section of an introductory course on logic design, it's important to understand that building a hardware accelerator requires a slightly different train of thought. It's not a *standalone piece of hardware* that we're implementing in the FPGA (e.g., a simple counter whose input is connected to a push button and output is displayed via a 7-segment display); we're designing an FPGA peripheral that's just *one small component of a much larger, complicated digital system*. In this case, we're going to design a hardware accelerator that performs Protocol Buffer serialization and communicates with the ARM Cortex-A9 CPU on board, which in turn executes a Protocol Buffer Linux application. The ARM CPU (master) executing the software initiates a sequence of write transactions sending data to the hardware accelerator (slave) that needs to be serialized. The hardware accelerator processes the data and stores it in an output buffer, ready to be accessed by the ARM CPU via read transactions. The hardware accelerator I just described serves as a co-processor to the ARM CPU, and its top-level inputs and outputs are solely connected to the (interconnect that establishes the connection with the) CPU! It may be troubling at frist to think you're going to build a very sophisticated digital circuit whose only means of interaction is via instructions executed on an ARM CPU (sorry, no <a href="https://en.wikipedia.org/wiki/DIP_switch">dip switches</a> or LEDs here!), but rest assured, I'll show you how to methodically approach the problem and make it seem less like a leap of faith.
