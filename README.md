...
*This is perhaps the most important step.* Time spent here will directly impact your approach to the problem, the design of your hardware accelerator, and ultimately your success in imporving system performance. A philosophy of mine is that one's understanding of how something works is proportional to that individual's ability to debug issues or improve upon its design. When you're attempting to replace components of a large software project with specialized hardware, this is especially true. The goal of this step is to fundamentally understand **the movement of and operations on data** and identify *performance bottlenecks* in the software. Is the system *memory bandwidth-limited*? Is it *computation-limited*? Answering these questions will give you insight into what can be tuned to improve its performance.

When working with someone else's software, extra time is needed to *understand how the code is structured* and to *identify source files, data structures, functions, etc.* that are relevant to the computation you wish to accelerate. This was my case in choosing to work with <a href="https://developers.google.com/protocol-buffers/">Protocol Buffers</a>; I had no prior experience using the software nor was I familiar with <a href="https://developers.google.com/protocol-buffers/docs/encoding">varint encoding</a> (a mechanism used heavily in the Protocol Buffer serialization code). The obvious first step was to acquire a basic understanding of how Protocol Buffers are used, and I achieved this by going through the online documentation, building and installing the software, and running the example application provided. After that, I was ready to dig deeper and understand specifically how the *Protocol Buffer serialization* code works.

There are powerful tools available that helped me identify the source code responsible for *Protocol Buffer serialization* and understand the serialization code to the extent at which I was able to begin designing the specialized hardware that would eventually replace it. One such tool is the marriage of <a href="http://www.vim.org/">vim</a> and <a href="http://ctags.sourceforge.net/">ctags</a>; I used this combination to review the example application, jumping between source files that contained the definitions of class methods as they were invoked. This helped me better understand the relationship between the compiler-generated code and `libprotobuf.so.10.0.0` runtime library that are components of any Protocol Buffer application. Using another tool, <a href="https://www.gnu.org/software/gdb/">gdb</a>, I then stepped through the same application and inspected <a href="https://en.wikipedia.org/wiki/Stack_trace">stack traces</a> as it actively serialized a Protocol Buffer <a href="https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.message#Message">Message</a>. This helped me understand the sequence (and frequency) of method invocations involved in serialization. The last tool I'll mention is <a href="https://perf.wiki.kernel.org/index.php/Main_Page">perf</a> - a powerful <a href="https://en.wikipedia.org/wiki/Profiling_(computer_programming)">profiling</a> tool used in <a href="https://en.wikipedia.org/wiki/Performance_tuning">performance tuning</a>. We use it to profile a Protocol Buffer applicaiton, acquiring its performance statistics and revealing bottlenecks in the software stack. `perf` can make use of <a href="https://en.wikipedia.org/wiki/Hardware_performance_counter">performance counters, kernel <a href="https://www.kernel.org/doc/Documentation/trace/tracepoints.txt">tracepoints</a>, even <a href="http://www.brendangregg.com/perf.html#USDT">user-level statically defined tracing (USDT)</a>, and more to periodically sample an application throughout its execution and provide detailed reports on where it spends its time (i.e., what percentage of the total execution time various code paths account for).
    
In the remainder of this section, I'll provide an overview of the Protocol Buffer software, demonstrate how I used the `vim`+`ctags` and `gdb` tools to identify source files, classes, and methods relevant to *Protocol Buffer serialization*, and discuss how time spent analyzing the `WireFormatLite` and <a href="https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.io.coded_stream#CodedOutputStream">CodedOutputStream</a> classes and their relation to the various <a href="https://developers.google.com/protocol-buffers/docs/proto3#scalar">field types</a> led to a key realization and simplifcation of the hardware accelerator design. I'll conclude the section with a brief discussion about importance of using `perf` at this stage.

#### Overview of the Protocol Buffer software
- Training
...
1. Download version 3.0.1 (?) of the <a href="https://github.com/google/protobuf">Protocol Buffer</a> source code.
