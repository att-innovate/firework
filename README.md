...
In this tutorial, my goal is to cover the development of a complete digital system, from setting up your development environment with the necessary EDA tools to designing a hardware accelerator (at the <a href="https://en.wikipedia.org/wiki/Register-transfer_level">RTL level</a>, implemented in an <a href="https://en.wikipedia.org/wiki/Field-programmable_gate_array">FPGA</a>), system integration (establishing communication with an ARM CPU, system memory, etc.), writing a device driver (providing <a href="https://en.wikipedia.org/wiki/User_space">user space</a> applications a kernel-level interface to the newly developed hardware) and ultimately, to modifying user space applications such that they utilize the hardware accelerator. While the knowledge gained and experience acquired will be extremely rewarding, undertaking a project of this size requires commitment, perseverance, and the ability to debug without the assistance of <a href="https://stackoverflow.com/tour">Stack Overflow</a> or similar forums. I emphasize the last requirement (and it's a good skill to have in general) for two reasons:
...
- [a10-soc-devkit-ghrd](a10-soc-devkit-ghrd): the **system design**. This repository contains the Arria 10 SoC <a href="https://rocketboards.org/foswiki/Documentation/A10GSRD151GHRDOverview">Golden Hardware Reference Design (GHRD)</a>, modified with the *protobuf-serializer* hardware accelerator added as a memory-mapped FPGA peripheral. This too is packaged as a Quartus Prime project with the main SoC subsystem being a Qsys system design (more on this later).
- <a href="https://github.com/altera-opensource/linux-socfpga">linux-socfpga</a>: the **Linux kernel**. This repository contains the Linux kernel source code, modified and maintained by Altera, for use with Arria 10 SoCs. We'll configure and compile the kernel from source to enable certain `CONFIG` options (e.g., `CONFIG_KALLSYMS` which provides kernel-level symbols) that are needed to profile the system.
- [driver](driver): the **device driver**. This repository contains the source code for a *platform device driver* for the *protobuf-serializer hardware accelerator*. This driver, implemented as a loadable kernel module, provides an interface to the newly developed hardware accelerator and specifies the necessary modifications to the *Protocol Buffer runtime library*. It's ultimately responsible for relaying data from user space memory to the hardware accelerator residing in the FPGA.
- [protobuf](protobuf): the **modified Protocol Buffer runtime library**. This repository is a fork of version `v3.0.2` of Google's <a href="https://github.com/google/protobuf">Protocol Buffers</a> open source project. The runtime library is modified to interact with the device driver, sending data from user space memory to the *protobuf-serializer* FPGA peripheral for serializing rather than the CPU.
- [profiling](profiling): the **test applications**, **profiling data**, and a **tutorial** on running <a href="http://www.brendangregg.com/perf.html">`perf`</a> directly on the Arria 10 SoC to understand system performance. The tutorial demonstrates how to profile test applications running on both, the *hardware-accelerated* and *standard* (software/CPU) systems, giving you the ability to benchmark baseline system performance and compare it to that of the hardware-accelerated system. Here we *close the loop*; that is, we finally see how a user space application is executed on top of the *standard* and *hardware-accelerated* systems.

Although Firework specifically covers the design of a hardware accelerator for *Protocol Buffer serialization* using an *Arria 10 SoC Development Kit*, I made an effort to generalize the hardware-accelerated system design process. The following sequence of high-level steps serve as a general approach to *any hardware accelerator project*, and the remainder of this tutorial provides in-depth coverage of each of these steps as they pertain to Firework.
...
In the last section, I claimed that understanding the software you wish to accelerate was *the most important step* of the entire process. Well, I think designing the hardware accelerator is *the most interesting (and challenging) step* as much of it relies on problem solving, piecing together information from various sources, and creativity. Before I could begin designing the hardware accelerator (i.e., the top-level I/O ports, FSMs, and datapath of the RTL design), I needed to first understand **how it would integrate into the larger Arria 10 SoC** such that I've established communication with the ARM Cortex-A9 CPU and could interact with the FPGA hardware via software running in a Linux environment. Knowing this would essentially define the hardware accelerator's top-level I/O ports (i.e., external facing signals) and leave the internal design to my creativity. I'll provide an overview of how it fits and how I figured that out next!

If you've worked with <a href="https://en.wikipedia.org/wiki/Field-programmable_gate_array">FPGAs</a> in the past to implement simple digital designs, perhaps in the lab section of an introductory course on logic design, it's important to understand that building a hardware accelerator requires a different train of thought. It's not a *standalone piece of hardware* that we're implementing in the FPGA (e.g., a counter whose input is connected to a push button and output is displayed via a <a href="https://en.wikipedia.org/wiki/Seven-segment_display">7-segment display</a>); we're designing an FPGA peripheral that's just *one small component of a much larger, complicated digital system*. Specifically, we're designing a hardware accelerator that performs *Protocol Buffer serialization* and communicates with an ARM Cortex-A9 CPU, where the ARM CPU in turn executes a Protocol Buffer application in a Linux environment. In this configuration, the hardware accelerator serves as a *co-processor* to the ARM CPU, and it's viewed as a *memory-mapped FPGA peripheral* from the ARM CPU's perspective. What differentiates this FPGA design from other standalone projects is that, aside from the complexity, its top-level I/O ports are solely connected to the <a href="https://www.altera.com/products/design-software/fpga-design/quartus-prime/features/qts-platform-designer.html">system interconnect</a> (i.e., system bus) which enables this peripheral's *discovery* and *access* by the ARM CPU. It may be troubling at frist to think you're going to build a very sophisticated digital circuit whose only means of interaction is via instructions executed on an ARM CPU (sorry, no <a href="https://en.wikipedia.org/wiki/DIP_switch">dip switches</a> or LEDs here!), but rest assured, I'll show you how to methodically approach the problem and make it seem less like a leap of faith, like it was for me.

How did I come to realize that this is how the hardware accelerator would eventually fit within the larger Arria 10 SoC? It required a lot of patience and piecing together information from various sources. The following is a list of separate initiatives I undertook that, although not comprehensive, I later identified as key contributors to obtaining the information I needed to begin designing the hardware accelerator:
- going through an extensive amount of Intel's <a href="https://www.altera.com/support/training/curricula.html">online training</a> which covers everything from the general FPGA design process to how their EDA tools are used at the various stages of design, simulation, and testing (functional verificaiton, static timing analysis)
- going through the <a href="https://rocketboards.org/foswiki/Documentation/AlteraSoCWorkshopSeries">Altera SoC Workshop Series</a> training on software development for Altera SoCs and learning how to write Linux device drivers for *memory-mapped FPGA peripherals*
- learning about the *FPGA-to-HPS*, *lightweight HPS-to-FPGA*, and *HPS-to-FPGA* interfaces that connect the Arria 10 SoC's <a href="https://www.altera.com/products/soc/portfolio/arria-10-soc/arria10-soc-hps.html">Hard Processor System (HPS)</a> (~synonymous to ARM CPU) to soft IP (i.e., FPGA peripherals) residing in the FPGA fabric. I also learned about the <a href="https://en.wikipedia.org/wiki/Address_space">address space</a> reserved for these interfaces and how one assigns addresses to FPGA components of a <a href="https://www.altera.com/products/design-software/fpga-design/quartus-prime/features/qts-platform-designer.html">Qsys system</a> such that the HPS could interact with them by reading and writing from and to these addresses
- fundamentally understanding the <a href="https://rocketboards.org/foswiki/Documentation/GSRDGhrd">Arria 10 GHRD</a> architecture and its design files, Quartus Prime and Qsys projects, example FPGA peripherals and how they're accessed from Linux. I also realized that the Arria 10 GHRD would be the starting point for my own *Qsys system design*
- one Intel online training in particular (purposely omitted here) that helped me connect all the information I had acquired up to this point and answered the question of how the hardware accelerator fits within the Arria 10 GHRD
- and finally, reading the <a href="https://github.com/att-innovate/firework/blob/master/resources/AMBA%20AXI%20and%20ACE%20Protocol%20Specification.pdf">AMBA AXI and ACE Protocol Specification</a> to learn about the *<a href="https://en.wikipedia.org/wiki/Bus_(computing)">bus protocol</a>* used by ARM-based SoCs and specifically, the signals that comprise *<a href="https://www.xilinx.com/products/intellectual-property/axi.html#details">AXI4 interfaces</a>* - the interface our hardware accelerator's top-level I/O ports would implement!

The remainder of this section is broken down into three parts. First, I'll discuss the relevant Intel online training and other available resources that led to understanding how the hardware accelerator would fit into the larger Arria 10 SoC. I'll conclude this subsection by identifying one course in particular that [put all the pieces together](README.md#iv-putting-the-pieces-together), allowing me to begin designing the hardware accelerator. Next, I'll present the RTL design of the hardware accelerator and describe its architecture, various pipeline stages, and how it implements an AXI4 slave interface for communication with the ARM CPU. Finally, I'll walk you through using Quartus Prime, ModelSim-Intel FPGA, and Qsys to implement the hardware accelerator (using <a href="https://en.wikipedia.org/wiki/Verilog#Verilog_2001">Verilog</a> to capture the RTL design) and run gate-level simulations for functional verification.
...
- the **HPS2FPGA** and other bridges (i.e., interfaces or communication channels) between the <a href="https://www.altera.com/products/soc/portfolio/arria-10-soc/arria10-soc-hps.html">Arria 10 HPS</a> and FPGA fabric
- and **Linux device drivers** for memory-mapped FPGA peripherals in Altera SoCs (covers general Linux driver development techniques and walks through example drivers written for the Arria 10 GHRD's FPGA peripherals)
...
Each workshop consists of *presentation material* (slides in the form of a .pdf file) and a *take-home lab*; don't bother with the labs - they're geared towards a different development board and this tutorial essentially replaces them anyway. While all three workshops provide useful information, there's a lot of overlap, irrelevant information, and information that applies to later steps in this tutorial, so I'll do my best to summarize key slides/info here. Regardless, it may be a good idea to skim through the presentations anyway, but don't worry about trying to understand every little detail. Note, we'll focus on **workshops 1 and 2** for now; **workshop 3 covers essential information for driver development** and is the main material of the later section, [7. Write a device driver (interface between FPGA peripheral and user space application)](README.md#7-write-a-device-driver-interface-between-fpga-peripheral-and-user-space-application).

Honestly, some of the information in these presentations is so fundamental to understanding *how to build FPGA designs that interact with the Arria 10 HPS* that it makes me wonder why it's buried in such an obscure location. If all the relevant information I've identified in this and other subsections of [a. Relevant training, examples, and other online resources](README.md#a-relevant-training-examples-and-other-online-resources) were succinctly contained in some `README` file, "Quick Start Guide", or a "What You Need to Know for Building FPGA Designs on the Arria 10 SoC" tutorial when you first open the box, it would've literally shaved months off of all the searching and prep work/learning I had to do before I could begin working on my design. Without further ado, here's the important information from workshops 1 and 2 that are relevant to building FPGA designs for the Arria 10 SoC.

Here are four different diagrams of the Arria 10 SoC architecture (why there are four, I'm not quite sure). Each one presents slightly different details about the various hardware blocks in the design, connections, and bus information. Let's see what we can learn from each and try to connect the dots:

![alt text](resources/images/arria-10-soc-1.png)

Hopefully at this point you're already familiar with the Arria 10 SoC's architecture. If not, spend some time <a href="https://www.altera.com/products/soc/portfolio/arria-10-soc/features.html">here</a> going over the Arria 10 SoC Block Diagram and list of its features. In the diagram above, we see the two main components of the Arria 10 SoC: the HPS (light blue) and FPGA (light green). We also see several hardware blocks that constitute the HPS. Enclosed in the red rectangle are the ones of interest: the **HPS to FPGA** and **FPGA to HPS** interfaces. Knowing that we'll eventually want the Protocol Buffer application (running in the HPS) to send data to our hardware accelerator (running in the FPGA) to serialize, it appears we'll have to send data over one or more of these interfaces. Let's look at the next diagram to see what else we can learn about them.

![alt text](resources/images/arria-10-soc-2.png)

In this diagram, we can infer more details about these interfaces. Starting at the **ARM Cortex-A9 MPCore** block, we see its **L2 Cache** is connected to an external block, the system **Interconnect**, and the **Interconnect** is in turn connected to the **HPS to FPGA** and **FPGA to HPS** interfaces (in this diagram, they incorrectly appear to be blocks of the FPGA, but don't worry about that). What does this tell us? If you're familiar with basic principles of <a href="https://www.amazon.com/Computer-Organization-Design-MIPS-Fifth/dp/0124077269">computer organization and design</a> and know a thing or two about <a href="https://en.wikipedia.org/wiki/Memory_hierarchy">memory hierarchies</a>, then it appears that the ARM CPUs running the Protocol Buffer applicaiton would have to communicate serialization data by *executing instructions that store data in the system's memory*. The data propegates through the CPU's L1 cache to the shared L2 chache, and instead of making its way to the **Multi-port DDR SDRAM Controller** to be stored in the system's main memory, it would be directed to the **Interconnect** and eventually to the FPGA via the **HPS to FPGA** interface. Alright, now we're getting somewhere! Let's see how we can build on this in the next diagram.

![alt text](resources/images/arria-10-soc-3.png)

Notice that in the previous two diagrams, there are *two arrows* extending from the **HPS to FPGA** interface; here we learn new details about it. We see that it actually consists of two separate interfaces: a (lightweight) **LW HPS to FPGA** and (full or high-performance) **HPS to FPGA** interface. Note, "CORE" is synonymous to "FPGA" in the diagram; they were lazy when labeling it. We're also presented new information about all three interfaces: 
...
<a href="https://en.wikipedia.org/wiki/Advanced_Microcontroller_Bus_Architecture#Advanced_eXtensible_Interface_(AXI)">AXI</a> 

configurable *32-*, *64-*, or *128-bit AXI* interface also operating at 300 MHz
