...
First, let's see how the six `CodedOutputStream::Write*()` methods identified in the last section are translated into a hardware accelerator design. Then, we'll integrate the newly developed FPGA peripheral into the larger <a href="https://rocketboards.org/foswiki/Documentation/GSRDGhrd">Arria 10 GHRD</a> system, where it serves as a co-processor to the ARM Cortex-A9 CPU on board.

### 4. Design and Implement the hardware accelerator (FPGA peripheral)

I claimed that last section was *the most important step* of the process. Well, I think this section is *the most interesting step* as much of it relies on *problem solving* and *creativity*. If you've used <a href="https://en.wikipedia.org/wiki/Field-programmable_gate_array">FPGAs</a> to build simple designs in the past, perhaps in the lab section of an introductory course on logic design, it's important to understand that building a hardware accelerator requires a different train of thought. It's not a *standalone piece of hardware* that we're implementing in the FPGA (e.g., a counter whose input is connected to a push button and output is displayed on a 7-segment display); we're designing an FPGA peripheral that's just *one small component of a much larger, complicated digital system*. Specifically, we're going to design a hardware accelerator that performs *Protocol Buffer serialization* and is capable of *communicating with an ARM Cortex-A9 CPU*, where the ARM CPU in turn executes a Protocol Buffer Linux application. The ARM CPU (**master**) initiates a sequence of *write transactions* sending data to the hardware accelerator (**slave**) for it to serialize. The hardware accelerator must also be capable of responding to subsequent *read transactions*, sending serialized data from its output buffer to the ARM CPU. In this configuration, the hardware accelerator serves as a *co-processor* to the ARM CPU and is viewed as an *FPGA peripheral* from its perspective. What differentiates this FPGA design from other standalone projects is that its top-level I/O ports are solely connected to the system interconnect which enables its *discovery* and *access* by the ARM CPU! It may be troubling at frist to think you're going to build a very sophisticated digital circuit whose only means of interaction is via instructions executed on an ARM CPU (sorry, no <a href="https://en.wikipedia.org/wiki/DIP_switch">dip switches</a> or LEDs here!), but rest assured, I'll show you how to methodically approach the problem and make it seem less like a leap of faith (like it was for me).

Getting to the point where I knew exactly how the hardware accelerator would eventually fit into the larger SoC such that it could interact with software running in a Linux environment required a lot of patience and piecing together information from various sources. I had to:
- go through an extensive amount of the <a href="https://www.altera.com/support/training/curricula.html">online training</a> that Intel provides
- become intimate with the EDA tools we'll be using and learn how and when they're used within the FPGA design process
- fundamentally understand the <a href="https://rocketboards.org/foswiki/Documentation/GSRDGhrd">Arria 10 GHRD</a> architecture and how its example FPGA peripherals are accessed from Linux
- learn about the FPGA-to-HPS, lightweight HPS-to-FPGA, and HPS-to-FPGA interfaces and the <a href="https://en.wikipedia.org/wiki/Address_space">address space</a> reserved for the <a href="https://rocketboards.org/foswiki/Documentation/GSRDGhrd?sortcol=0;table=1;up=0#sorted_table">HPS-to-FPGA interface</a> for accessing FPGA peripherals (soft IP) from the Arria 10 SoC <a href="https://www.altera.com/products/soc/portfolio/arria-10-soc/arria10-soc-hps.html">Hard Processor System (HPS)</a>
- go through the <a href="https://rocketboards.org/foswiki/Documentation/AlteraSoCWorkshopSeries">Altera SoC Workshop Series</a> to learn about software development for Altera SoCs and see how device drivers are written for memory mapped FPGA peripherals
- and finally, read the <a href="https://github.com/att-innovate/firework/blob/master/resources/AMBA%20AXI%20and%20ACE%20Protocol%20Specification.pdf">ARM AMBA AXI protocol specification</a> in order to understand the signals that comprise the hardware accelerator's top-level I/O ports and establish communication with ARM CPU.

First, let's look at the Intel online training, examples, and other resources available that were necessary to reach the point of understanding how the hardware accelerator fits in to the larger Arria 10 SoC system. Then, I'll go over the RTL design in quite some detail, explaining how it implements an ARM AMBA AXI4 slave interface and is ready to communicate with an ARM CPU. Finally, we'll see how to use Intel's EDA tools to implement the hardware accelerator design and run gate-level simulations for functional verification. 
