...
- and finally, reading the <a href="https://github.com/att-innovate/firework/blob/master/resources/AMBA%20AXI%20and%20ACE%20Protocol%20Specification.pdf">AMBA AXI and ACE Protocol Specification</a> to learn about the <a href="https://en.wikipedia.org/wiki/Bus_(computing)">bus protocol</a> used by ARM-based SoCs and specifically, the signals that comprise AXI4 interfaces (a [specific course](README.md#putting-the-pieces-together) from Intel's online training led me to realize that the hardware accelerator's top-level I/O ports need to define an AXI4 slave interface in order to establish communication with the ARM CPU, an AXI4 master)

The remainder of this section is broken down into three parts. First, I'll list the relevant Intel online training, examples, and other available resources that led to understanding how the hardware accelerator would fit into the larger Arria 10 SoC. I'll conclude this subsection by identifying the [one course in particular](README.md#putting-the-pieces-together) that put all the pieces together and allowed me to begin designing the hardware accelerator. Next, I'll describe the RTL design of the hardware accelerator in quite some detail and how it implements an ARM AMBA AXI4 slave interface to establish communication with an ARM CPU. Finally, I'll walk you through using Quartus Prime, ModelSim-Intel FPGA, and Qsys to implement the hardware accelerator (using <a href="https://en.wikipedia.org/wiki/Verilog#Verilog_2001">Verilog</a> to capture the RTL design) and run gate-level simulations for functional verification.

#### a. Relevant training, examples, and other online resources

The <a href="https://www.altera.com/support/training/curricula.html">Intel FPGA Technical Training</a> provides free online courses on almost every aspect of the FPGA design process and using their EDA tools at the various stages. Of the courses available, below are the ones that I found relevant for this project. I recommend going through them as needed; if you're already familiar with a particular topic or tool then feel free to skip the training. At a minimum, I recommend going through the bolded courses as a refresher on how to use Quartus Prime, ModelSim-Intel FPGA Edition, and Qsys.
...
Performing static timing analysis using the TimeQuest Timing Analyzer
