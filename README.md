...
When working with someone else's software, extra time is needed to *understand how the code is structured* and to *identify source files, data structures, functions, etc.* that are relevant to the computation you wish to accelerate. This was the case for me in choosing to work with <a href="https://developers.google.com/protocol-buffers/">Protocol Buffers</a>; I had no prior experience using the software nor was I familiar with <a href="https://developers.google.com/protocol-buffers/docs/encoding">varint encoding</a> - a mechanism used heavily in the Protocol Buffer serialization code. After first going through online documentation, building and installing the software, and running an example application to obtain a basic understanding of how Protocol Buffers are used, I was ready to dig deeper. There are powerful tools available that help you build the intuition needed to pinpoint code of interest and eventually replace it with specialized hardware. One such tool is the marriage of <a href="http://www.vim.org/">vim</a> + <a href="http://ctags.sourceforge.net/">ctags</a>; I used this combo to walk through the example applicaiton and jump between source files containing definitions of class methods as they were invoked. This helped me better understand the relationship between the user space application, compiler-generated code, and `libprotobuf.so.10.0.0` runtime library that are components of any Protocol Buffer application. I then used another tool, <a href="https://www.gnu.org/software/gdb/">gdb</a>, to step through the same application and inspect the <a href="https://en.wikipedia.org/wiki/Stack_trace">stack trace</a> as it actively serialized a Protocol Buffer <a href="https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.message#Message">Message</a>. This further cemented my understanding and helped me identify the class methods responsible for serialization. <a href="https://perf.wiki.kernel.org/index.php/Main_Page">perf</a> is another powerful tool that can be used to <a href="https://en.wikipedia.org/wiki/Profiling_(computer_programming)">profile</a> an applicaiton. It uses <a href="https://en.wikipedia.org/wiki/Hardware_performance_counter">performance counters, kernel <a href="https://www.kernel.org/doc/Documentation/trace/tracepoints.txt">tracepoints</a>, etc. to collect and provide detailed information on where an application spends its time (i.e., which function(s) are called more/less frequently in its execution) which useful information for performance tuning. 
