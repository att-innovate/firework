...
Earlier, I mentioned that the ARM CPU sends data to the hardware accelerator by executing instructions to store data in the system's memory. From its perspective, any address it writes to belongs to the system's <a href="https://en.wikipedia.org/wiki/Computer_data_storage#Primary_storage">main memory</a>. That is, it's completely unaware of the **L1** and **L2 Cache**, **L3 Interconnect**, **SDRAM Controller Subsystem** and even the fact that it's using <a href="https://en.wikipedia.org/wiki/Virtual_address_space">virtual addresses</a> which the SDRAM controller translates into physical addresses (i.e. memory locations) of the <a href="https://en.wikipedia.org/wiki/Synchronous_dynamic_random-access_memory">SDRAM</a> chips that constitute the system's main memory. Next, let's see how data is directed to FPGA periperals over the **Lightweight HPS-to-FPGA bridge** and **HPS-to-FPGA bridge** rather than system's main memory.
