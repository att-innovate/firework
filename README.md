...
There are powerful tools available that helped me pinpoint the source code that performs *Protocol Buffer serialization* and understand it to a level at which I was able to begin designing the hardware that would eventually replace it. One such tool is the marriage of <a href="http://www.vim.org/">vim</a> and <a href="http://ctags.sourceforge.net/">ctags</a>; I used this combination to review an example application line by line, jumping between the source files containing the definitions of class methods as they were invoked. This helped me better understand the relationship between the *compiler-generated code* and *runtime library* that are components of any Protocol Buffer application. I used the <a href="https://www.gnu.org/software/gdb/">GNU Debugger</a>, `gdb`, to step through the same application and inspect the <a href="https://en.wikipedia.org/wiki/Stack_trace">stack trace</a> as it actively serialized a Protocol Buffer <a href="https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.message#Message">Message</a>. This helped me understand the sequence (and frequency) of method invocations involved in serialization. The last tool I'll mention is <a href="https://perf.wiki.kernel.org/index.php/Main_Page">perf</a> - a powerful <a href="https://en.wikipedia.org/wiki/Profiling_(computer_programming)">profiling</a> tool used in <a href="https://en.wikipedia.org/wiki/Performance_tuning">performance tuning</a>. `perf` makes use of <a href="https://en.wikipedia.org/wiki/Hardware_performance_counter">performance counters, kernel <a href="https://www.kernel.org/doc/Documentation/trace/tracepoints.txt">tracepoints</a>, even <a href="http://www.brendangregg.com/perf.html#USDT">user-level statically defined tracing (USDT)</a> and more to periodically sample a running application and provide detailed reports on where it spends its time (i.e., the various code paths taken and percentage of total execution time they account for). As demonstrated in the section [Profiling the HW-accelerated system](README.md#9-profiling-the-hw-accelerated-system), we use it to profile a Protocol Buffer applicaiton running on the standard (purely CPU/software model) and HW-accelerated systems, acquire performance statistics, and analyze differences in its execution on the two systems.
    
In the remainder of this section, I'll provide an overview of the Protocol Buffer software, demonstrate how I used the `vim`+`ctags` and `gdb` tools to identify source code relevant to *Protocol Buffer serialization*, and discuss how time spent analyzing the `WireFormatLite` and <a href="https://developers.google.com/protocol-buffers/docs/reference/cpp/google.protobuf.io.coded_stream#CodedOutputStream">CodedOutputStream</a> classes and various <a href="https://developers.google.com/protocol-buffers/docs/proto3#scalar">field types</a> led to a key realization and simplifcation of the hardware accelerator design. I'll conclude the section with a brief discussion about importance of using `perf` at this stage too (a lesson that I learned).

#### Overview of Protocol Buffers
From the <a href="https://developers.google.com/protocol-buffers/docs/overview">Developer Guide</a>, "Protocol buffers are a flexible, efficient, automated mechanism for serializing structured data". In the world of Protocol Buffers, data structures are called **messages**, and they contain one or more key/value pairs called **fields**, similar to <a href="http://www.json.org/">JSON objects</a>. The idea is that you define the messages you want to use in your application once in a `.proto` file and use the Protocol Buffer compiler (`protoc`) to generate specialized code for these messages in the language of your choice (e.g., C++ classes). The compiler-generated code provides accessors for each field and works closely with the Protocol Buffer *runtime library*, `libprotobuf.so.10.0.0`, to serialize/parse entire messages to/from streams or storage containers. Protocol Buffers are *extensible* in the sense that you can add new fields to messages without disrupting existing applications that use older formats; this is achieved by marking new fields as `optional` instead of `required`.

For a more complete understanding of what Protocol Buffers are and how they're used, and to learn about varint encoding and how messages are serialized, go through the following material. This material also serves as a prerequisite for the remaining material in this section and subsequent sections in the tutorial:
...
As we've learned, field keys consist of **field numbers** or **tags** and a **wire type**. 

Using the address book example application from the C++ tutorial, let's serialize an `AddressBook` containing one `Person` message by hand to become more familiar with how messages are serialized. Our example `Person` message contains the following field values:
- `name`:   Kevin Durant
- `id`:     35
- `email`:  kd@warriors.com
- `phones`: `number`: 4155551988, `type`: MOBILE
