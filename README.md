...
I claimed that last section was *the most important step* of the entire process. Well, I think this section is *the most challenging and interesting step* as much of it relies on piecing together information from various sources, problem solving, and creativity. Before I could begin designing the hardware accelerator (i.e., top-level I/O, FSMs, datapath), using the `CodedOutputStream::Write*()` methods as templates, I needed to first understand **how the FPGA design would integrate into the larger Arria 10 SoC** such that I've established communication with the ARM Cortex-A9 CPU and could interact with it via software running in a Linux environment. I'll explain that next.

If you've worked with <a href="https://en.wikipedia.org/wiki/Field-programmable_gate_array">FPGAs</a> in the past to build simple digital circuits, perhaps in the lab section of an introductory course on logic design, it's important to understand that building a hardware accelerator requires a different train of thought. It's not a *standalone piece of hardware* that we're implementing in the FPGA (e.g., a counter whose input is connected to a push button and output is displayed on a <a href="https://en.wikipedia.org/wiki/Seven-segment_display">7-segment display</a>); we're designing an FPGA peripheral that's just *one small component of a much larger, complicated digital system*. Specifically, we're designing a hardware accelerator that performs *Protocol Buffer serialization* and is capable of communicating with an ARM Cortex-A9 CPU, where the ARM CPU in turn executes a Protocol Buffer application in a Linux environment. In this configuration, the hardware accelerator serves as a *co-processor* to the ARM CPU and is viewed as an *FPGA peripheral* from its perspective. What differentiates this FPGA design from other standalone projects is that, aside from the complexity, its top-level I/O ports are solely connected to the <a href="https://www.altera.com/products/design-software/fpga-design/quartus-prime/features/qts-platform-designer.html">system interconnect</a> which enables its *discovery* and *access* by the ARM CPU! It may be troubling at frist to think you're going to build a very sophisticated digital circuit whose only means of interaction is via instructions executed on an ARM CPU (sorry, no <a href="https://en.wikipedia.org/wiki/DIP_switch">dip switches</a> or LEDs here!), but rest assured, I'll show you how to methodically approach the problem and make it seem less like a leap of faith, like it was for me.

How did I come to realize this is how the hardware accelerator would fit into the larger Arria 10 SoC? It required a lot of patience and piecing together information from various sources. Not a comprehensive list but rather initiatives later identified as contributing to obtaining this understanding, I had to:
- go through an extensive amount of Intel's <a href="https://www.altera.com/support/training/curricula.html">online training</a> covering everything from the general FPGA design process to how their EDA tools are used at the various stages of design, simulation, and testing (functional verificaiton, static timing analysis)
- fundamentally understand the <a href="https://rocketboards.org/foswiki/Documentation/GSRDGhrd">Arria 10 GHRD</a> architecture and how the provided example FPGA peripherals are accessed from Linux, and realize this would be the starting point for my *Qsys system design*
- learn about the *FPGA-to-HPS*, *lightweight HPS-to-FPGA*, and *HPS-to-FPGA* interfaces which connect the Arria 10 SoC's <a href="https://www.altera.com/products/soc/portfolio/arria-10-soc/arria10-soc-hps.html">HPS</a> to soft IP (i.e., FPGA peripherals) in its FPGA fabric along with the <a href="https://en.wikipedia.org/wiki/Address_space">address spaces</a> reserved for them and how to assign addresses to components of a <a href="https://www.altera.com/products/design-software/fpga-design/quartus-prime/features/qts-platform-designer.html">Qsys system</a> 
- go through the <a href="https://rocketboards.org/foswiki/Documentation/AlteraSoCWorkshopSeries">Altera SoC Workshop Series</a> to learn about software development for Altera SoCs and learn how Linux device drivers are written for memory-mapped FPGA peripherals in the Arria 10 Soc
- and finally, read the <a href="https://github.com/att-innovate/firework/blob/master/resources/AMBA%20AXI%20and%20ACE%20Protocol%20Specification.pdf">ARM AMBA AXI protocol specification</a> in order to understand the signals that would comprise the hardware accelerator's top-level I/O ports and establish communication with ARM CPU

The remainder of this section is broken down into three main subsections. First, I'll provide a list of the relevant Intel online training, design examples, and other available resources that were necessary for me to understand how the hardware accelerator would fit into the larger Arria 10 SoC. I'll conclude this subsection by discussing one training in particular that put all the pieces together and finally allowed me to begin designing the hardware accelerator. Next, I'll discuss the RTL design of the hardware accelerator in quite some detail and describe how it implements an ARM AMBA AXI4 slave interface, establishing communication with an ARM CPU. Finally, I'll walk you through using Intel's EDA tools to implement the hardware accelerator RTL design and run gate-level simulations for functional verification.

#### Training that led to understanding how the hardware accelerator must be designed to fit into the Arria 10 GHRD system
